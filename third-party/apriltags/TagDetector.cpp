...
TagDetectorParams::TagDetectorParams() : sigma(kDefaultSigma),
...
TagDetector::TagDetector(const TagFamily &f,
                         const TagDetectorParams &parameters) : tagFamily(f),
                                                                params(parameters),
                                                                debug(kDefaultDebug),
  ...
  if (true)
  {
    ...
    for (int y = 2; y + 2 < fimseg.rows; y++)
    {
      for (int x = 2; x + 2 < fimseg.cols; x++)
      {
        ...
        // DOWN & LEFT
        edgeCost = (x == 0) ? -1 : this->edgeCost(theta0, mag0, fimTheta(y + 1, x - 1), fimMag(y + 1, x - 1));
                          ...
                          TagDetectionArray &detections) const
{
  ...
  if (debug)
  {
    debugShowQuads(images, quads, 7, "Quads");
  }
  ...
  if (params.refineQuads)
  {
    ...
    if (debug)
    {
      debugShowQuads(images, quads, 8, "Refined quads");
    }
  }
                             ...
                             TagDetectionArray &detections) const
{
  ...
  for (int iy = -1; iy <= dd; iy++)
  {
    for (int ix = -1; ix <= dd; ix++)
    {
      ...
      at::real v = NEW_QUAD_ALGORITHM ? bicubicInterpolate(images.origBW8, pxy) * gscl : bicubicInterpolate(images.fim, pxy);
