...
//大致意思好像是传个字符串，然后初始化字符串所代表的tagfamily
...
void TagFamily::decode(TagDetection& det, TagFamily::code_t rcode) const {
  ...
  //codes代表family里的所有编码，也就是说问题就在codes所拥有的编码里，
  //只要把codes的编码换成26个英文字母所代表的编码就可以识别
  for (size_t id = 0; id < codes.size(); id++) {
    ...
    for (int rot = 0; rot < 4; rot++) {
      ...
      if (thishamming < besthamming) {
        // std::cout<<"this hamming = "<<thishamming<<" best hamming = "<<besthamming<<" code = "<<codes[id]<<std::endl;
  ...
  det.obsCode = rcode;
  det.code = bestcode;
  // std::cout<<"id = "<<det.id<<" code = "<<det.code<<std::endl;
} 
